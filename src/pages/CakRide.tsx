...
import { Plus, Minus, Bike, Loader2 } from 'lucide-react';
...
const CakRide: React.FC = () => {
  ...
  useEffect(() => {
    getTariff('cakride').then(setTariff);
    const saved = localStorage.getItem('cakjek_user_name');
    if (saved) setName(saved);
  }, []);

  const total = tariff && distance != null
    ? Math.round(tariff.base_fare + tariff.per_km * distance)
    : null;
  ...
  const calcDistance = async () => {
    ...
    try {
      ...
      for (let i = 0; i < waypoints.length - 1; i++) {
        dist += await routeDistanceKm(waypoints[i], waypoints[i + 1]);
      }
  ...
  useEffect(() => { if (pickup && stops[0]?.coords) calcDistance(); }, [pickup, stops]); // eslint-disable-line
...
Dikirim via ${settings.app_name}`;
    ...
    try {
      const res = await createOrder({
        service: 'cakride', customer_name: name, customer_phone: '',
        details: {
          pickup_coords: { lat: pickup.lat, lng: pickup.lng },
          pickup_address: pickup.address,
          stops: stops.map((s) => ({ address: s.coords?.address, lat: s.coords?.lat, lng: s.coords?.lng })),
          distance, note,
        },
        total, message: msg, status: 'new',
      });
      if (res) { setWaUrl(res.whatsapp_url); setSuccess(true); addNotification('cakride', 'Pesanan ojek online berhasil dikirim ke admin'); }
    } catch (err) {
  ...
  return (
    <div>
      <ServiceHeader
        title="Ojek Online"
        subtitle={tariff ? `Mulai ${formatIDR(tariff.base_fare)} + ${formatIDR(tariff.per_km)}/km` : 'Motor cepat & terjangkau'}
      ...
      <div className="-mt-8 mx-4 relative z-10 space-y-3">
          ...
          {stops.map((s, i) => (
            <div key={i} className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                {stops.length > 1 && (
                  <button type="button" onClick={() => setStops((prev) => prev.filter((_, j) => j !== i))} className="text-xs text-destructive">Hapus</button>
                )}
              </div>
              <GoogleMapsPicker
                label={`Lokasi Tujuan ${stops.length > 1 ? i + 1 : ''}`}
                value={s.coords}
                onChange={(c) => setStops((prev) => prev.map((st, j) => j === i ? { coords: c } : st))}
              />
            </div>
          ))}
        ...
        <div className="bg-card rounded-3xl p-5 shadow-sm border border-border/50">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-semibold">
              {loading ? <Loader2 size={14} className="animate-spin" /> : distance != null ? `±${distance} km` : '-'}
            </span>
          </div>
          {tariff && distance != null && (
            <div className="flex justify-between text-sm mb-2">
              <span className="text-muted-foreground">Tarif</span>
              <span>{formatIDR(tariff.base_fare)} + {formatIDR(tariff.per_km)} × {distance} km</span>
            </div>
          )}
          <div className="flex justify-between font-extrabold text-lg border-t border-border pt-3 mt-2">
            <span>Total</span>
            <span className="text-primary">{total != null ? formatIDR(total) : '-'}</span>
          </div>
        </div>
        ...
        <button
          onClick={handleOrder}
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 bg-[#128C7E] hover:bg-[#075E54] text-white font-extrabold py-4 rounded-3xl shadow-lg shadow-green-900/20 transition active:scale-[0.98] disabled:opacity-60"
        >
