...
type MartForm = Omit<MenuItem, 'id' | 'created_at'>;
const EMPTY: MartForm = { name: '', price: 10000, description: '', image_url: '', category: 'mart', merchant_id: null, active: true };

const AdminMenuMart: React.FC = () => {
  const [items, setItems] = useState<MenuItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState<MenuItem | null>(null);
  const [form, setForm] = useState<MartForm>(EMPTY);

  const load = () => { setLoading(true); adminGetMenuItems('mart').then(setItems).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);

  const openAdd = () => { setEditing(null); setForm(EMPTY); setModal(true); };
  const openEdit = (m: MenuItem) => { setEditing(m); setForm({ name: m.name, price: m.price, description: m.description, image_url: m.image_url, category: 'mart', merchant_id: null, active: m.active }); setModal(true); };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (editing) { await updateMenuItem(editing.id, form); toast.success('Produk diperbarui'); }
      else { await createMenuItem(form); toast.success('Produk ditambahkan'); }
      closeModal(); load();
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteMenuItem(id); toast.success('Produk dihapus'); load(); } catch { toast.error('Gagal menghapus'); }
  };

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-extrabold">CakMart — Produk</h2><p className="text-sm text-muted-foreground">{items.length} produk</p></div>
          ...
          <table className="w-full min-w-max">
            <thead className="border-b border-border bg-muted/50">
              <tr className="text-xs text-muted-foreground">
                <th className="text-left p-3 font-medium whitespace-nowrap">Produk</th>
            ...
            <tbody>
              {loading ? ([...Array(3)].map((_, i) => <tr key={i}><td colSpan={4} className="p-3"><div className="h-8 bg-muted rounded animate-pulse" /></td></tr>))
                : items.length === 0 ? (<tr><td colSpan={4} className="text-center text-muted-foreground py-10 text-sm">Belum ada produk</td></tr>)
                : items.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition">
                  <td className="p-3 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      {item.image_url ? <img src={item.image_url} alt={item.name} className="w-9 h-9 rounded-xl object-cover shrink-0" /> : <div className="w-9 h-9 rounded-xl bg-muted grid place-items-center shrink-0"><span>🛒</span></div>}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-card rounded-3xl shadow-2xl w-full max-w-[calc(100%-2rem)] md:max-w-lg max-h-[90dvh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h3 className="font-bold">{editing ? 'Edit Produk' : 'Tambah Produk'}</h3>
              <button onClick={closeModal} className="w-8 h-8 rounded-full bg-muted grid place-items-center"><X size={15} /></button>
            </div>
            <form onSubmit={handleSave} className="p-5 space-y-4">
              <ImageInput label="Foto Produk" value={form.image_url} onChange={(url) => setForm((f) => ({ ...f, image_url: url }))} />
              <FF label="Nama Produk"><input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nama produk..." className="input-field" required /></FF>
