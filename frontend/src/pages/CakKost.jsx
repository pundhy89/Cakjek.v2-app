...
export default function CakKost() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", move_in: "", months: 1, notes: "" });
  ...
  const open = (k) => {
    setSelected(k);
    setForm({ name: "", phone: "", move_in: new Date().toISOString().slice(0, 10), months: 1, notes: "" });
  };
  const close = () => setSelected(null);

  const total = selected ? Number(selected.price_month || 0) * Math.max(1, Number(form.months || 1)) : 0;

  const submit = async () => {
    if (!form.name || !form.phone || !form.move_in) {
      toast.error(t(lang, "fill_required"));
      return;
    }
    setLoading(true);
    const message = `Halo Admin CakJek,\nSaya mau booking *CakKost*.\n\nNama: ${form.name}\nNo. HP: ${form.phone}\n\nKost: ${selected.name}\nAlamat: ${selected.address}\nFasilitas: ${selected.facilities}\nHarga / bulan: ${formatIDR(selected.price_month)}\n\nMulai sewa: ${form.move_in}\nDurasi: ${form.months} bulan\nCatatan: ${form.notes || "-"}\n\nTotal: ${formatIDR(total)}`;
    try {
      const r = await api.post("/orders", {
        service: "cakkost",
        customer_name: form.name,
        customer_phone: form.phone,
        details: { kost_id: selected.id, kost_name: selected.name, move_in: form.move_in, months: Number(form.months), notes: form.notes, price_month: selected.price_month },
        total,
        message,
      });
      ...
      <div className="px-5 -mt-6">
        <div className="bg-card rounded-3xl border border-black/5 dark:border-white/10 p-3 shadow-md space-y-3">
          {items.map((k) => (
            ...
            >
              {k.image ? (
                <img src={k.image} alt={k.name} className="w-20 h-20 rounded-xl object-cover" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-emerald-600 mt-1">{formatIDR(k.price_month)} <span className="text-[10px] text-muted-foreground font-medium">/bulan</span></p>
              </div>
            </button>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={close}>
          <div onClick={(e) => e.stopPropagation()} className="bg-card rounded-t-3xl sm:rounded-3xl w-full max-w-md p-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-4 space-y-2.5">
              ...
              <In label="Tanggal Mulai Sewa" type="date" v={form.move_in} on={(v) => setForm({ ...form, move_in: v })} testid="kost-date" />
              <In label="Durasi (bulan)" type="number" v={form.months} on={(v) => setForm({ ...form, months: v })} testid="kost-months" />
            ...
            <div className="border-t border-border mt-4 pt-3 flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">{form.months} bln × {formatIDR(selected.price_month)}</p>
